...
<body>
  ...
  try {
    ...
    $stmt = $conn->prepare("SELECT * FROM tb_class WHERE id_class = :idcls");
    $stmt->bindParam(':idcls', $idcls);
    $stmt->execute();
  
    if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
      $category = $row['nm_class'];
      $description = $row['ds_class'];
      $cdps = $row['cd_ps'];
  ...
  <div class="sidebar-content offcanvas offcanvas-start d-flex flex-column flex-shrink-0 p-3" tabindex="-1" id="offcanvasExample" aria-labelledby="offcanvasExampleLabel">
    ...
    <ul class="nav flex-column gap-3">
      <li>
        <a class="nav-link d-flex align-items-center border rounded-2" data-bs-toggle="collapse" href="#multiCollapseExample1" role="button" aria-expanded="false" aria-controls="multiCollapseExample1">
      ...
      <li>
        <a class="bg-active text-white nav-link d-flex align-items-center border rounded-2" data-bs-toggle="collapse" href="#multiCollapseExample2" aria-expanded="false" aria-controls="multiCollapseExample2">
  ...
  <nav class="navbar shadow navbar-dark bgsucces fixed-top">
    <div class="header-body container-fluid d-flex">
      ...
      <h1 class="page-name">Minha Loja</h1>
